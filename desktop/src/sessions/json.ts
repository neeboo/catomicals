 ...
 * Ported from DeepSeek Harness `packages/core/session/src/json.ts` (MIT) —
 * the iterative validator/snapshotter is faithful so the persisted boundary
 * rejects the same exotic values DSH rejects.
 *
 * @module catomicals-desktop/sessions/json
 */
...
/**
 * Validate and detach lossless JSON in one read per property. Rejects sparse,
 * cyclic, exotic, negative-zero, and non-finite values.
 * @returns the detached snapshot, or `undefined` when not losslessly serializable.
 */
export function snapshotJsonValue<T>(value: T): T | undefined {
  return walkJsonValue(value, true) as T | undefined;
}

/** Test the same lossless JSON boundary without detaching. */
