...
describe("Cordis fixed plugin host", () => {
  it("isolates a bad package without blocking a valid plugin", async () => {
    ...
    expect(host.listPlugins()).toEqual(expect.arrayContaining([
  ...
  it("isolates a plugin with a missing required service", async () => {
    ...
    expect(host.readHealth(fixture.registration.id)).toMatchObject({
      status: "isolated",
  ...
  it("validates patches and creates an intent without mutating last-good settings", async () => {
    ...
    expect(host.listPlugins()).toContainEqual(expect.objectContaining({ pluginId: fixture.registration.id, status: "ready" }));
  ...
  it("keeps the last-good tree when migration fails", async () => {
    ...
    expect(host.readHealth(fixture.registration.id)).toMatchObject({ status: "isolated", code: "migration_failed" });
