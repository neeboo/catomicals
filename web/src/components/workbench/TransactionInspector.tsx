...
function ReviewResult({ review }: { review: TransactionReview }) {
  return (
    <section className="transaction-review" aria-live="polite">
      ...
      ) : <p className="review-clear">钱包节点未返回警告</p>}

      <details className="review-details">
        <summary>查看资金流</summary>
        <div className="flow-list">
          {review.inputs.map((input) => (
            <div key={`input-${input.index}`}><span>输入 {input.index}</span><strong>{input.value_sat.toLocaleString()} sat</strong></div>
          ))}
          {review.outputs.map((output) => (
            <div key={`output-${output.index}`}><span>输出 {output.index}</span><strong>{output.value_sat.toLocaleString()} sat</strong></div>
          ))}
        </div>
      </details>
    </section>
...
export function TransactionInspector() {
  ...
  function request(): TransactionReviewRequest | null {
    try {
      ...
      setFormError(inspectionError(error));
      return null;
    }
  }
  ...
  function submit(event: FormEvent) {
    ...
    if (!payload) return;
    inspect.mutate(payload, { onError: (error) => setFormError(inspectionError(error)) });
  }

  return (
    <div className="inspector-scroll">
      <p className="plugin-description">钱包节点重新解析交易并自行计算 BIP341 签名摘要；这里不接受手填摘要。</p>
      <form className="inspector-form" onSubmit={submit}>
        <label>
          <span>未签名交易</span>
          <textarea rows={5} value={rawTxHex} onChange={(event) => setRawTxHex(event.target.value)} placeholder="粘贴 raw transaction hex" spellCheck={false} />
        </label>
        <label>
          <span>按输入顺序排列的 Prevouts</span>
          <textarea rows={5} value={prevoutsJson} onChange={(event) => setPrevoutsJson(event.target.value)} spellCheck={false} />
        </label>
